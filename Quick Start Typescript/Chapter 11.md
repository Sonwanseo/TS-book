...
제네릭 클래스에 전달된 매개변수가 클래스이고 타입 매개변수일 때 코드 어시스트를 받지 못할 때가 있음  
이유는 타입 매개변수 <T>에 타입이 없기 때문  
만약 코드 어시스트 지원을 받고고 더욱 명시적으로 타입을 선언하려면 <T extends 인터페이스>처럼 클래스에 대한 인터페이스를 상속

##### 특정 메서드만 제네릭 메서드로 사용하기

제네릭 클래스를 사용하]면 클래스 전역에 걸쳐 타입 매개변수가 적용  
만약 특정 메서드만을 대상으로 제네릭을 적용하려면 해당 메서드를 제네릭 메서드로 선언

메서드 단위로 제네릭을 적용하는 것은 특정 메서드에만 타입 인수를 전달할 수 있으므로 타입의 재활용성이 떨어짐  
그런데 클래스 단위로 제네릭을 적용하면 클래스 내에 존재하는 불특정 메서드에 대해 일괄적으로 제네릭 메서드로 선언할 수 있어서 더 편리

## 11.4 제네릭의 여러 활용 방법

### 11.4.1 룩업 타입을 제네릭 클래스에 적용

룩업 타입은 keyof로 속성을 포함하는 대상을 탐색해 유니언 타입처럼 동작
...
타입 매개변수 K는 타입 매개변수 T에 의해 정해지는 룩업 타입이 됨  
따라서 타입 매개변수 K는 keyof를 통해 타입 매개변수 T의 속성을 탐색해 하나의 속성만 허용하도록 제약

### 11.4.2 인터페이스를 상속해 제네릭 확장하기

인터페이스는 클래스가 구현해야 할 메서드나 프로퍼티 선언 가능  
만약 구현해야 할 클래스가 제네릭 클래스라면 인터페이스는 제네릭 인터페이스로 선언해야 함

### 11.4.3 맵 객체의 선언과 타입 지정 방법

자바스크립트의 맵 객체는 타입을 지정할 수 없어 타입 안정성이 없었음  
반면 타입스크립트에서는 맵 객체에 타입을 지정할 수 있어 타입 안정성이 있음

##### 맵 객체의 선언과 사용 방법

맵은 키-값을 관리하는 데 있어서 효율적인 자료구조
...
맵 객체는 순회 가능한 컬렉션이므로 for of 문을 통해 순회할 수 있음

맵 객체는 입력된 키-값 객체를 처리할 수 있는 여러 메서드 지원  
책 382p 참고

##### 맵 객체 사용 시 타입을 지정하기

타입스크립트는 맵 객체에 타이을 지정할 수 있게 해 타입 안정성을 줄 수 있음

```typescript
let list: Map<number, string> = new Map<number, string>();
```

맵 객체를 할당받을 변수에 맵 객체의 타입이 선언돼 있다면 타입 안정성이 보장되므로 맵 객체를 생성할 때 타입 인수를 생략해도 좋음

### 11.4.4 제네릭 기반의 자료구조 만들기

맵 객체는 내장 객체로 지원되지만 스택, 큐, ArrayList 등과 같은 자료구조는 내장 객체로 지원되지 않음  
개발 과정에서 맵 객체 외에 다른 자료구조가 필요하다면 제네릭을 이용해 정의하는 게 좋음
